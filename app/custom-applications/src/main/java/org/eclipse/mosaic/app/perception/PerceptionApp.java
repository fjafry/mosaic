 ...
 public class PerceptionApp extends AbstractApplication<VehicleOperatingSystem> implements VehicleApplication {
     ...
     private void perceiveVehicles() {
        ...
        if(perceivedVehicles.size() > 0) {
            getLog().infoSimTime(this, "First perceived vehicle: {}", perceivedVehicles.get(0).getId());
            getLog().infoSimTime(this, "at position: x: {}, y: {}", perceivedVehicles.get(0).getProjectedPosition().getX(), perceivedVehicles.get(0).getProjectedPosition().getY());
        }
        //  final String pos = perceivedVehicles.stream().map(VehicleObject::getProjectedPosition);
